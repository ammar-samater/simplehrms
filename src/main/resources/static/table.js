    ...
    for (i = 0; i < dataLength; i++) {
        ...
        if (isEditable || isDeletable) {
            ...
            if (isEditable) {
                ...
                icon1.addEventListener('click', createShowUpdateFormFunction(parent, structure, data[i].id, reload)); 
            }

            if (isDeletable) {
                var icon2 = createOptionIcon("deleteIcon", ["fa", "fa-trash-o"]);
                icon2.addEventListener('click', createSendAjaxRequestFunction(
                   structure.url + "/" + data[i].id, "DELETE",
                    function () {
...
function createShowUpdateFormFunction(parent, structure, id, reload) {
	 return function () {
	    	getViewSection(parent, structure.options.update.updateForm.replace("/id", "/" + id),  function () { 
 ...
 * @returns
 */
function createShowAddFormButton(parent, structure, reload) {
    var addButton = document.createElement('button');
    addButton.innerHTML = "Add";
    addButton.classList.add("add-btn");

    var showAddFormFunction = createShowAddFormFunction(parent, structure, reload);
 ...
 * @returns
 */
function createShowAddFormFunction(parent, structure, reload) {
    return function () {
